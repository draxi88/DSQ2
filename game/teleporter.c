...
void SP_LaserSight(edict_t *self) {
	...
	lss->s.modelindex = gi.modelindex("models/objects/dmspot/tris.md2");
	lss->s.skinnum = 0;
...
void LaserSightThink(edict_t *self)
{
	...
	VectorSet(angle, 0, 0, -90);
	VectorAdd(tr.plane.normal, angle, tr.plane.normal);
	vectoangles(tr.plane.normal, self->s.angles);
	VectorSet(angle, 10, 10, 0);
	VectorSubtract(tr.endpos, angle, tr.endpos);
